...
import { ILinkInputProps } from "@/components/types";
...
const LinkInput = forwardRef<ImperativeHandle, ILinkInputProps>(
      ...
      <div className="text-input-container">
        {label && (
          <label
            ...
            htmlFor={restProps.id || `text-input-link-${label}`}
          >
            {label}
          </label>
        )}
        <div
          className={`link-input-main ${isFilled ? "filled" : ""} ${
            hasError || isError ? "error" : ""
          }`}
        >
          <span className="link-default-text">http://</span>
          <div
            className={`text-input link ${isFilled ? "filled" : ""} ${
              hasError || isError ? "error" : ""
            }`}
          >
            <input
              ...
              id={restProps.id || `text-input-link-${label}`}
